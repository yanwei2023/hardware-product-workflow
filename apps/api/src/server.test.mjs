...
test("health endpoint reports the active project", async () => {
  ...
  assert.equal(result.headers["access-control-allow-headers"], "content-type,x-request-id,x-pilot-access-code");
...
test("runtime config endpoint reports non-secret deployment settings", async () => {
  ...
  assert.equal(result.body.runtimePersistence.startupCheck.required, false);
