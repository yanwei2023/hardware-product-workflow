...
test("compose waits for PostgreSQL and exposes its internal connection to the app", () => {
  ...
  assert.match(compose, /HARDWARE_FLOW_RUNTIME_WRITE_MODE: \$\{HARDWARE_FLOW_RUNTIME_WRITE_MODE:-auto\}/);
  assert.match(compose, /HARDWARE_FLOW_RUNTIME_PERSISTENCE_BACKEND: \$\{HARDWARE_FLOW_RUNTIME_PERSISTENCE_BACKEND:-json\}/);
  assert.match(compose, /HARDWARE_FLOW_RUNTIME_POSTGRES_SYNC_DIR: \/app\/data\/runtime-postgres-sync/);
