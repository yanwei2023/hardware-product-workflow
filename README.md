...
- 项目归档/恢复、角色负责人变更、工作包计划日期更新、工作包证据引用新增、Agent 作业排队、Agent 队列作业处理、Agent 输出记录、人工审核提交、有条件批准条款完成、当前阶段风险创建、风险接受/关闭、风险缓解计划更新、缓解完成、阶段门批准、单条通知已读和项目内用户通知批量已读已接入原生 PostgreSQL 增量事务链路：业务状态、审计事件和站内通知在同一事务提交或更新，并执行全表写后校验；校验漂移时运行补偿事务恢复旧值并删除本次审计/通知/批准包/证据引用/风险行/Agent 作业行/Agent 运行行/交付物草稿行。其他业务写入暂时继续使用精确镜像。
