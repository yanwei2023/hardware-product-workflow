...
- 标准项目创建、项目导入/克隆、项目归档/恢复、权限拒绝审计、角色负责人变更、阶段门就绪状态刷新、工作包计划日期更新、工作包证据引用新增、Agent 作业排队、Agent 输出记录、人工审核提交、有条件批准条款完成、当前阶段风险创建、风险接受/关闭、风险缓解计划更新、缓解完成、阶段门批准、单条通知已读和项目内用户通知批量已读已接入原生 PostgreSQL 增量事务链路：业务状态、审计事件和站内通知在同一事务提交或更新，并执行全表写后校验；校验漂移时运行补偿事务恢复旧值并删除本次审计/通知/批准包/证据引用/风险行/Agent 作业行/Agent 运行行/交付物草稿行。
- 项目选择只更新 JSON store 中的 `activeProjectId`，不会改变 PostgreSQL 业务行；`postgres-mirror` 模式会把它记录为 local-only mutation，并通过 `hardware_flow_runtime_local_only_mutations_total` 指标暴露。
- 存储备份恢复、检查点恢复和演示数据重置属于运维级全量替换路径，在 `postgres-mirror` 模式下继续使用精确镜像和全表写后校验，并通过 `hardware_flow_runtime_operational_full_replacements_total` 与 exact mirror 计数区分。
