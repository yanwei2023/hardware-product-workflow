...
其中“试点演练清单”会按当前项目数据列出检查点、角色负责人、工作包排期、Agent 草稿、人类审核、风险处置、通知、审计和归档包等事项的 DONE/PENDING 状态，可作为试点主持人的现场脚本。
...
如果试点前已经创建检查点，优先在页面“项目 -> 本地数据状态”点击“恢复最新检查点”。恢复前系统会保留当前文件的 `.pre-restore-*.bak` 副本，方便二次排查。
