...
镜像模式下，角色负责人变更、工作包计划日期更新、风险接受/关闭、风险缓解计划更新和缓解完成已经使用原生 PostgreSQL 增量事务，将业务状态、审计和通知一起提交；其他修改仍使用全量精确镜像。页面“本地数据状态”和 `/metrics` 会显示启动一致性、当前持久化就绪状态、最近数据库写入模式及两类事务累计次数。
