...
镜像模式下，角色负责人变更、工作包计划日期更新、工作包证据引用新增、Agent 作业排队、人工审核提交、有条件批准条款完成、当前阶段风险创建、风险接受/关闭、风险缓解计划更新、缓解完成、阶段门批准、单条通知已读和项目内用户通知批量已读已经使用原生 PostgreSQL 增量事务，将业务状态、风险行、Agent 作业行、证据引用、审计、通知和阶段门批准包一起提交或更新；其他修改仍使用全量精确镜像。页面“本地数据状态”和 `/metrics` 会显示启动一致性、当前持久化就绪状态、最近数据库写入模式及两类事务累计次数。
