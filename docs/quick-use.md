...
镜像模式下，角色负责人变更、工作包计划日期更新、工作包证据引用新增、人工审核提交、风险接受/关闭、风险缓解计划更新、缓解完成、阶段门批准和单条通知已读已经使用原生 PostgreSQL 增量事务，将业务状态、证据引用、审计、通知和阶段门批准包一起提交或更新；其他修改仍使用全量精确镜像。页面“本地数据状态”和 `/metrics` 会显示启动一致性、当前持久化就绪状态、最近数据库写入模式及两类事务累计次数。
