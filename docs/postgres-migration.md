...
- 第一条原生增量事务覆盖角色负责人变更。`role_pairs` 更新、`ROLE_PAIR_UPDATED` 审计事件和相关通知在同一 PostgreSQL 事务内提交，并使用旧负责人作为并发前置条件；提交后仍执行全表比较，发现漂移时执行补偿事务恢复旧负责人并删除本次新增审计/通知。补偿失败会锁定后续写入并让 readiness 失败，直到人工修复并重启通过一致性门禁。状态接口和指标分别统计增量事务与精确镜像成功次数，尚未迁移的业务动作继续走全量镜像。
