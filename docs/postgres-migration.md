...
- 原生增量事务目前覆盖角色负责人变更和工作包计划日期更新。`role_pairs.human_user_id` 或 `work_packages.due_at` 更新、对应审计事件和相关通知在同一 PostgreSQL 事务内提交，并使用旧值作为并发前置条件；日期比较使用空值安全条件。提交后仍执行全表比较，发现漂移时执行补偿事务恢复旧值并删除本次新增审计/通知。补偿失败会锁定后续写入并让 readiness 失败，直到人工修复并重启通过一致性门禁。状态接口和指标分别统计增量事务与精确镜像成功次数，尚未迁移的业务动作继续走全量镜像。
