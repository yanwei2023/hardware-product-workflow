...
- 原生增量事务目前覆盖角色负责人变更、工作包计划日期更新、工作包证据引用新增、Agent 作业排队、人工审核提交、有条件批准条款完成、当前阶段风险创建、风险接受/关闭、风险缓解计划更新、风险缓解完成、阶段门批准、单条通知已读和项目内用户通知批量已读。`role_pairs.human_user_id`、`work_packages.due_at`、`work_package_evidence_refs` 插入、`agent_jobs` 排队插入、`reviews` 插入及相关工作包/交付物状态、`reviews` 有条件批准完成字段、`risks` 插入及对应风险决策/缓解字段更新、阶段门批准涉及的 `gates`、`phases`、`projects`、`gate_approval_packs`，或 `notifications.status/read_at` 更新，以及对应审计事件和相关通知，会在同一 PostgreSQL 事务内提交，并使用旧值作为并发前置条件；日期和已读时间比较使用空值安全条件，批量通知已读会限定在同一项目和用户范围内逐行更新。提交后仍执行全表比较，发现漂移时执行补偿事务恢复旧值并删除本次新增审计/通知/批准包/证据引用/风险行/Agent 作业行。补偿失败会锁定后续写入并让 readiness 失败，直到人工修复并重启通过一致性门禁。状态接口和指标分别统计增量事务与精确镜像成功次数，尚未迁移的业务动作继续走全量镜像。
...
- 角色、排期、证据引用、Agent 作业排队、工作包审核、风险处置主链路、阶段门批准和通知已读已经进入原生增量事务；后续继续把 Agent 输出、项目生命周期等剩余写入从全量镜像收敛到仓储级 PostgreSQL 写入。
