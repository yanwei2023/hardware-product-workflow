...
- 原生增量事务目前覆盖角色负责人变更、工作包计划日期更新、工作包证据引用新增、人工审核提交、风险接受/关闭、风险缓解计划更新、风险缓解完成和阶段门批准。`role_pairs.human_user_id`、`work_packages.due_at`、`work_package_evidence_refs` 插入、`reviews` 插入及相关工作包/交付物状态、`risks` 中对应风险决策/缓解字段更新，或阶段门批准涉及的 `gates`、`phases`、`projects`、`gate_approval_packs`，以及对应审计事件和相关通知，会在同一 PostgreSQL 事务内提交，并使用旧值作为并发前置条件；日期比较使用空值安全条件。提交后仍执行全表比较，发现漂移时执行补偿事务恢复旧值并删除本次新增审计/通知/批准包/证据引用。补偿失败会锁定后续写入并让 readiness 失败，直到人工修复并重启通过一致性门禁。状态接口和指标分别统计增量事务与精确镜像成功次数，尚未迁移的业务动作继续走全量镜像。
