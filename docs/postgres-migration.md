...
- 角色、排期、证据引用、Agent 作业排队、Agent 输出、工作包审核、风险处置主链路、阶段门批准和通知已读已经进入原生增量事务；后续继续把项目生命周期等剩余写入从全量镜像收敛到仓储级 PostgreSQL 写入。
